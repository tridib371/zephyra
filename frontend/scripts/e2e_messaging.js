...
async function run() {
    try {
        ...
        const convRes = await axios.post(`${API_BASE}/messages/conversations/${userB.id}`, {}, { headers: { Authorization: `Bearer ${tokenA}` } });
        ...
        console.log('A sends a message');
        await axios.post(`${API_BASE}/messages/conversations/${conversation._id}`, { text: 'Hello from A' }, { headers: { Authorization: `Bearer ${tokenA}` } });
